...
class VisualizerSyncTests(SimpleTestCase):
    ...
    @patch("zmq.Context")
    def test_api_link_stream_view(self, mock_zmq_context):
        ...
        # Make recv_string yield a test message, then raise ValueError to break the infinite loop
        test_payload = '{"bpm": 128.0, "beat": 12.0, "timestamp": 1687299500.0}'
        mock_socket.recv_string = MagicMock(side_effect=[test_payload, ValueError("Stop loop")])
        
        # Request the URL using the Django test client
